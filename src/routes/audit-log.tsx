  ...
  const groups = useMemo<AuditGroup[]>(() => {
    ...
    for (const row of sorted) {
      ...
      if (current) {
        ...
        const withinWindow = new Date(row.created_at).getTime() - new Date(lastRow.created_at).getTime() <= GROUP_WINDOW_MS;
      ...
      <Card>
        ...
        <CardContent>
            ...
            <Table>
              ...
              <TableBody>
                ...
                {!isLoading && pageGroups.length === 0 && (
                  <TableRow><TableCell colSpan={5} className="py-10 text-center text-sm text-muted-foreground"><Inbox className="mx-auto h-6 w-6 text-muted-foreground" /><p className="mt-2">No activity matches this filter.</p></TableCell></TableRow>
                )}
                      ...
                      >
                        <TableCell>
                          <div className="flex items-center gap-2.5">
                            ...
                            <Avatar className="h-7 w-7"><AvatarFallback className="text-[10px]">{initialsFromEmail(first.user_email)}</AvatarFallback></Avatar>
                            <div>
                              <p className="max-w-[140px] truncate text-sm font-medium leading-tight">{first.user_email || "—"}</p>
                              <p className="text-[11px] text-muted-foreground leading-tight">{first.role ? (ROLE_LABELS[first.role as Role] ?? first.role) : "—"}</p>
                            </div>
                          </div>
                        </TableCell>
                        <TableCell>
                          <div className="flex items-center gap-1.5">
                            <Badge variant={actionColor(first.action)}>{actionLabel(first.action)}</Badge>
                            {isMulti && (
                              <span className="rounded-full bg-muted px-1.5 py-0.5 text-[10px] font-medium text-muted-foreground">{group.events.length}×</span>
                            )}
                          </div>
                        </TableCell>
                        <TableCell className="text-sm">
                          {fmtDate(firstDt)} · {fmtTime(firstDt)}{isMulti ? `–${fmtTime(lastDt)}` : ""}
                        </TableCell>
                        <TableCell className="hidden md:table-cell text-xs text-muted-foreground">{deviceOf(first)} · {browserOf(first)}</TableCell>
                        <TableCell className="hidden lg:table-cell text-xs text-muted-foreground">{first.ip_address || "—"}</TableCell>
                      </TableRow>
                      {isMulti && isOpen && [...group.events].reverse().map((ev) => (
                        <TableRow key={ev.id} className="bg-muted/30 hover:bg-muted/40">
                          <TableCell colSpan={2} className="py-1.5 pl-11 text-xs text-muted-foreground">{new Date(ev.created_at).toLocaleTimeString()}</TableCell>
                          <TableCell className="py-1.5 text-xs text-muted-foreground" />
                          <TableCell className="hidden md:table-cell py-1.5 text-xs text-muted-foreground" />
                          <TableCell className="hidden lg:table-cell py-1.5 text-xs text-muted-foreground">{ev.ip_address || "—"}</TableCell>
                        </TableRow>
                      ))}
                    </Fragment>
                  );
          ...
          <div className="mt-4 flex items-center justify-end gap-3 text-sm text-muted-foreground">
            <span>Page {page} of {totalPages} ({groups.length} entries, {filtered.length} events)</span>
