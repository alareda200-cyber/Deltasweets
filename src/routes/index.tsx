...
import {
  linesQuery, entriesQuery, entryDowntimesQuery,
  productionAreasQuery, areaOwnersQuery, entryAreaOwnersQuery,
  departmentsQuery, departmentCategoriesQuery, downtimeTypesQuery, severityLevelsQuery,
} from "@/lib/queries";
...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Live MTD + last-day plant performance across all production lines." },
    ],
  }),
  loader: ({ context }) =>
    Promise.all([
...
function Dashboard() {
  ...
  async function handleExportPdf() {
    ...
    try {
      ...
      void logAudit("dashboard.export_pdf", "dashboard", activeLine?.id, { lineName: activeLine?.name, from, to });
    } catch (err) {
    ...
    <AppShell>
      {lines.length === 0 ? (
        ...
        <>
          ...
          <div className="mt-6 flex flex-col gap-4 rounded-2xl border border-border bg-card p-4 shadow-card md:flex-row md:items-end md:justify-between">
            <Tabs value={lineId} onValueChange={setLineId} className="flex-1">
              <TabsList className="flex w-full flex-wrap justify-start gap-1 bg-muted/50 p-1">
                {lines.map((l) => (
                  <TabsTrigger key={l.id} value={l.id} className="data-[state=active]:bg-card data-[state=active]:shadow-sm">
            ...
            <div className="flex flex-wrap items-end gap-3">
              ...
              <Button variant="outline" size="sm" onClick={() => { const m = monthRange(); setFrom(m.from); setTo(m.to); }}>
                This Month
              </Button>
          ...
          {activeLine && <DashboardBody lineId={activeLine.id} color={activeLine.color} from={from} to={to} containerRef={dashboardRef} />}
        </>
      )}
    </AppShell>
  );
...
function HeroHeader({ line, from, to }: { line: { name: string; color: string } | undefined; from: string; to: string }) {
  return (
...
function DashboardBody({ lineId, color, from, to, containerRef }: { lineId: string; color: string; from: string; to: string; containerRef: React.RefObject<HTMLDivElement | null> }) {
  const { role } = useAuth();
  ...
  if (entries.length === 0) {
    return (
      <div className="mt-6 rounded-2xl border border-dashed border-border bg-card p-12 text-center">
        ...
        <p className="mt-1 text-sm text-muted-foreground">Add a daily entry to populate the dashboard.</p>
      </div>
    );
  }

  return (
    <div ref={containerRef} className="mt-6 grid grid-cols-1 gap-6">
      <PerformanceSection
...
function EmptyState({ onCreate }: { onCreate: () => void }) {
  return (
    <div className="rounded-2xl border border-dashed border-border bg-card p-16 text-center">
      ...
      <p className="mt-2 text-sm text-muted-foreground">Create your first production line in Settings.</p>
      <Button className="mt-6" onClick={onCreate}>Open Settings</Button>
    </div>
  );
}
